    ...
    while (!pointsStack.empty()) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (estimatedPoint.x >= 0 && estimatedPoint.y >= 0 && estimatedPoint.x < src.cols && estimatedPoint.y < src.rows) {
                ...
                auto delta = pow(srcCenterPoint[0] - srcEstimatedPoint[0], 2) + pow(srcCenterPoint[1] - srcEstimatedPoint[1], 2) + pow(srcCenterPoint[2] - srcEstimatedPoint[2], 2);
...
int main() {
    ...
    return regionGrowingColor(img, 100, .01, 10);
}
